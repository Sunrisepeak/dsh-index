...
**单装一个 plugin 之后它不进任何 profile。** 原子层不碰 profile 是这次重构的核心 —— 实测证明了它的必要性：原子自己注册时，装一个 Agent 会连带把它的五个成员塞进 `web`，因为每个成员在 Agent 运行之前就已经替自己决定了归属。而 xlings 的依赖是递归调用 CmdProcessor 装的，被装的包拿不到"谁请求了我"（`_RUNTIME` 里只有自身与其依赖，无 parent/root），所以"直装时注册、被引用时不注册"在今天的 xlings 上无法表达。

代价是"我只想加一个小插件"的人要多一步。缓解有两条：安装后打印的是**完整可粘贴的命令**（tarball 路径或 pin 死的 git spec 都是已知的），以及未镜像的包也会用 `pnpm store add <git spec>` 预热 pnpm 的内容寻址 store，所以那一步是本地解析而不是再下载一次。

顺带一个收益：`pnpm store add` 只取不构建，所以本索引不再执行任何上游代码，`DSH_ALLOW_BUILDS` 这个自有开关也随之取消 —— 真正的门是 pnpm 的 `allowBuilds`，它守在代码真正会执行的那一步（用户组合时）。自己再造一道，只是那道门的弱化副本。
